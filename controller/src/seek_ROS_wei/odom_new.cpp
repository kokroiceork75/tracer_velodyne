...
#define load_data_FC "controller/search_target_controller/FUZZY-PID/temp_w-new.txt" //important controller.
...
/////////////////////////////  抓取機器人位置  //////////////////////////{
  ...
  // }
  void Final_Goal(const geometry_msgs::PoseStamped::ConstPtr & F_goal) // wei change,not sure
  {
      position_x1 = F_goal->pose.position.x;
      position_y1 = F_goal->pose.position.y;
      ...
      if (yaw_degrees >= -180 && yaw_degrees < -90) {
          orientation_z1 = yaw_degrees + 180; // 左下象限
      } else if (yaw_degrees >= -90 && yaw_degrees < 0) {
          orientation_z1 = yaw_degrees + 90;  // 右下象限
      } else if (yaw_degrees >= 0 && yaw_degrees < 90) {
          orientation_z1 = yaw_degrees - 90;  // 右上象限
      } else { // yaw_degrees >= 90 && yaw_degrees <= 180
          orientation_z1 = yaw_degrees - 270; // 左上象限
      }
  ...
  void amcl_Callback(const::move_base_msgs::MoveBaseActionFeedback::ConstPtr & amcl) // wei change,not sure
  {
    amcl_position_x = amcl->feedback.base_position.pose.position.x;
    amcl_position_y = amcl->feedback.base_position.pose.position.y;
    ...
    // 進行座標系轉換
    if (yaw_degrees >= -180 && yaw_degrees < -90) {
        amcl_orientation_z = yaw_degrees + 180; // 左下象限
    } else if (yaw_degrees >= -90 && yaw_degrees < 0) {
        amcl_orientation_z = yaw_degrees + 90;  // 右下象限
    } else if (yaw_degrees >= 0 && yaw_degrees < 90) {
        amcl_orientation_z = yaw_degrees - 90;  // 右上象限
    } else { // yaw_degrees >= 90 && yaw_degrees <= 180
        amcl_orientation_z = yaw_degrees - 270; // 左上象限
    }

    // 確保結果在 -180 到 180 度範圍內
    amcl_orientation_z = fmod(amcl_orientation_z + 180, 360) - 180;

    // 將結果轉回弧度
    amcl_orientation_z = amcl_orientation_z * M_PI / 180.0;
  }
...
void decideRotation( int &kkk){
  ...
  {  
    ...
    { 
      ...
      if(intergral_1>15) intergral_1=0;
      ...
      if (abs(error)<0.1745) {   //5.7degree 內直線行走
        intergral=0;
        intergral_1=0;
        printf("直走\n");
        status=1;
      ...
      speed_limit();
      v1=out_y[1]*0.098/1.5;
      v2=out_y[2]*0.098/1.5;
